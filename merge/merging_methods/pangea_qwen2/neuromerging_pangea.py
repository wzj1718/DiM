...
class NeuroTaskArithmeticPangeaQwen2:
    ...
    def ties_kernel(pt_w, ft_ws):
        ...
        final_signs = NeuroTaskArithmeticPangeaQwen2.resolve_sign(ft_ws_flat)

        merged_tv = NeuroTaskArithmeticPangeaQwen2.disjoint_merge(
    ...
    def ours_kernel(pt_w, ft_ws, key, cum_threshold=1):
        ...
        # projection merge
        scale = NeuroTaskArithmeticPangeaQwen2.inner_ties_kernel(scale)
        ...
        for m in range(scales.size(0)):
            scales_.append(
                NeuroTaskArithmeticPangeaQwen2.inner_ties_kernel(scales[m])
            )
    ...
    def merge(BASE_MODEL_PATH, VL_MODEL_PATH, EMMA_MODEL_PATH, save_path, **kwargs):
        ...
        vl_model = LlavaQwenForCausalLM.from_pretrained(VL_MODEL_PATH)
        ...
        for i in range(len(vl_model.model.layers)):
            ...
            for name, param in vl_layer.named_parameters():
                ...
                if param.data.ndim == 2:
                    # Linear weights
                    merged_delta = NeuroTaskArithmeticPangeaQwen2.ours_kernel(
                        base_param.data,
                        [delta_vl, delta_emma],
                        name
                    )
                    ...
                    merged_delta = NeuroTaskArithmeticPangeaQwen2.ties_kernel(
                        base_param.data,
                        [delta_vl, delta_emma]
                    )
        ...
        # merged_norm = NeuroTaskArithmeticPangeaQwen2.ties_kernel(
        #     base_norm,
        #     [vl_norm - base_norm, emma_norm - base_norm]
        # )

        # vl_model.model.norm.weight.data.copy_(
        #     base_norm + merged_norm
        # )
