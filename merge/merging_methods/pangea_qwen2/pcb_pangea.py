...
def pcb_merge_logic(task_vectors, pcb_ratio=0.1):
        ...
        # abs_vectors = clamp(torch.abs(task_vectors))
        # clamped_vectors = torch.sign(task_vectors) * abs_vectors
...
class PCBMergerPangeaQwen2():
    ...
    def merge(BASE_MODEL_PATH, VL_MODEL_PATH, EMMA_MODEL_PATH, save_path, **kwargs):
        ...
        vl_model = LlavaQwenForCausalLM.from_pretrained(VL_MODEL_PATH, torch_dtype=torch.float32, device_map="cpu")
        ...
        if vision_tower is not None:
            ...
            print("Vision tower meta params:", len(meta_params))
            if meta_params:
                raise RuntimeError(
                    "Vision tower still has meta parameters after load_model(): "
                    f"{meta_params[:20]}"
                )
