...
def disjoint_merge(Tensor, merge_func, sign_to_mult, lambdas=None):
    ...
    elif merge_func == "weighted":
        if lambdas is None:
            raise ValueError("lambdas must be provided for weighted merge")

        assert len(lambdas) == Tensor.shape[0]
        weights = torch.tensor(lambdas, device=Tensor.device).unsqueeze(1)
        disjoint_aggs = torch.sum(selected_entries * weights, dim=0)
    else:
